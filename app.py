...
def load_data():
    ...
    abc_series = pd.Series('B - Серебро', index=product_profit.index)
    abc_series[top30.index] = 'A - Золото'
    abc_series[c_mask] = 'C - Балласт'
...
# =========================================================
# YEAR SUMMARY (динамика только если выбрано ≥ 2 года)
# =========================================================
...
with col1:
    with st.container(border=True, key="plot1"):
        ...
        # Создаём список позиций для меток
        if total_months <= 12:
            tick_spacing = 1
        ...
        # Берём каждую N-ю дату для меток
...
with col1:
    with st.container(border=True, key="plot7"):
        ...
        fig.update_layout(height=400, xaxis=dict(title='Скидка (%)', tickformat='.0%', range=[-0.05, 0.85]), yaxis=dict(title='Прибыль'), coloraxis_showscale=False, margin=dict(l=20, r=20, t=20, b=20))
        st.plotly_chart(fig, width='stretch', config=plotly_config)
with col2:
    with st.container(border=True, key="plot8"):
        st.markdown('##### Топ-20 клиентов')
        cs = df.groupby('Customer ID').agg(Customer_Name=('Customer Name', 'first'),
                                           Total_Sales=('Sales', 'sum'),
                                           Total_Profit=('Profit', 'sum'),
                                           Orders=('Order ID', 'nunique')).reset_index()
        tc = cs.nlargest(20, 'Total_Sales')
        fig = px.bar(tc, x='Total_Sales', y='Customer_Name', orientation='h',
                     color='Total_Profit', template=plotly_template,
                     color_continuous_scale=['red', 'yellow', 'green'],
                     labels={'Total_Sales': 'Итого покупок', 'Customer_Name': ''})
        fig.update_traces(text=tc['Total_Sales'].apply(lambda x: f'{x:,.0f}'),
...
with col1:
    with st.container(border=True, key="plot9"):
        ...
        ad = df.groupby('ABC').agg(Products=('Product Name', 'nunique'), Sales=('Sales', 'sum'),
            ...
            ['A - Золото', 'B - Серебро', 'C - Балласт']).fillna(0)
        ...
        gold_sales = ad.loc['A - Золото', 'Sales']
        silver_sales = ad.loc['B - Серебро', 'Sales']
        bronze_sales = ad.loc['C - Балласт', 'Sales']
        ...
        if total_sales > 0:
            ...
            segments = []
            if gold_pct >= 0.1:
                segments.append((gold_pct, '#22c55e', '🥇 Золото'))
            if silver_pct >= 0.1:
                segments.append((silver_pct, '#f59e0b', '🥈 Серебро'))
            if bronze_pct >= 0.1:
                segments.append((bronze_pct, '#ef4444', '🥉 Балласт'))
        ...
        st.caption('A — топ-30 продуктов по прибыли | B — остальные прибыльные | C — убыточные')
with col2:
...
with st.container(border=True, key="plot11"):
    ...
    try:
        ...
        # Описание метода
        st.markdown("""
        <div style="font-size:13px; color:#475569; line-height:1.7; margin-top:12px;">
        ...
        • Модель раскладывает историю продаж на три компонента: <b>тренд</b> (рост/падение), <b>сезонность</b> (повторяющиеся паттерны) и <b>уровень</b> (базовая линия).<br><br>
        • Обучается на данных за выбранные годы и предсказывает на 12 месяцев вперёд — на 2018 год.<br><br>
        • Чем дальше горизонт прогноза — тем выше неопределённость. Модель не учитывает внешние факторы: маркетинговые акции, кризисы, изменение цен.<br><br>
        • Рекомендуется обновлять прогноз ежемесячно при поступлении новых данных для повышения точности.
        </div>
        """, unsafe_allow_html=True)

        # Бэктестинг только если данных >= 24 месяца
        if len(mv) >= 24:
